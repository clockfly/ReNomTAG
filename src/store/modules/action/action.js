...
let action = {
  ...
  reload_sidebar_thumbnail_and_filename_list (context, {current_page, page_step}) {
    ...
    return axios.post('/api/get_filename_list', fd).then(
      function (response) {
        ...
        let sidebar_file_name_list = response.data.filename_list
        fd.append('filename_list', sidebar_file_name_list)
        fd.append('current_page', current_page)
        fd.append('page_step', page_step)
